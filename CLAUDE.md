...
윈도우↔맥을 오간 레포라 **파일명 검색이 두 겹으로 거짓말한다.** 문서가 없다고 판정하기 전에 둘 다 확인할 것.
1. **한글 파일명 NFC/NFD 정규화** — 맥은 NFD로 저장. `find -name "한글*"`·`grep`이 **멀쩡히 있는 파일을 못 찾는다.** → `python3` + `unicodedata.normalize('NFC', ...)` 로 비교.
2. **윈도우 8.3 단축명 + 대문자 확장자** — `kiddy_voice/`에 이름이 뭉개진 파일 4개: `KI009F~1.MD` · `KI7A19~1.MD` · `그림일기/KI366E~1.MD` · `그림일기/KI7155~1.MD`. ASCII 이름이라 **한글로 못 찾고**, 확장자가 대문자 `.MD`라 **`--include="*.md"` 필터에서 통째로 빠진다.**
   - `그림일기/KI7155~1.MD` = **그림일기 설계 v2 (SoT)**. AD 계열 브리프·`ROADMAP-고도화.md`가 verbatim 참조하는 현행 문서다.
- **판정 규칙: 이름으로 못 찾으면 `*~1*.MD` 4개의 내용을 열어보고, 그래도 없으면 `git log --all --diff-filter=D` 로 이력까지 확인한 뒤에 "없다"고 말할 것.**
...
### ⚠️ LLM 반응(받아주기)에는 답의 '틀(시제·의미)'을 라벨로 못박을 것 (사고 발생: 볼 것 "동화 봤구나")
- **아이가 고른 답이 '한 일(과거)'인지 '보고 싶은 것(미래)'인지 LLM은 문맥만으로 헷갈린다.** 라벨(예: "보고 싶은 것")만으로는 부족 — 시제/의미 틀을 프롬프트에 명시하지 않으면 미래를 과거로 뒤집는다.
  - 사고: 볼 것 답 '동화'(=앞으로 보고 싶은 것)를 키디가 **"동화 봤구나"**(이미 봤다)로 반응. 특히 calm 예시가 과거형 활동("놀았구나")뿐이라 그 패턴을 볼 것에 그대로 적용.
- **재발방지 규칙:**
  1. 질문 유형별 **시제/틀을 프롬프트에 명시**할 것(예: `checkins.py`의 `FRAME_HINT` — 한 일=과거 "~했구나" / 볼 것=미래 "~보고 싶구나", "절대 '봤다'로 말하지 마").
  2. **예시(few-shot)도 유형별로 정확히** — 한 예시(과거)를 모든 유형에 재사용하면 그 시제가 오염된다.
  3. 로컬 폴백 템플릿이 항상 정확하면(예: `WATCH_REACTION`), 의심 시 **LLM 출력 대신 폴백**을 쓰는 서버 안전망도 고려(값싼 방어).
