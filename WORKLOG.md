...
- 🔒 **재발방지 장치(포카요케) 가동 중** — **커밋하면 자동으로 검사**한다. `npm install` 만 하면 훅이 자동 활성화되므로 **오너가 할 일은 없다.**
  - 백엔드(3.4초): 서버 임포트 · **인증 기준선** · **인가(소유권)** · 🐤 **윤리선 카나리아** · 💸 **비용 상한** → `cd server && ./venv/bin/python _verify_all.py`
...
- **다음 무게중심**: ① **S2b 나머지 레이트리밋**(chat·tts·reports·schedules) ② **S3b CORS `*`** (⚠️ 모바일 실기기 테스트와 얽힘) ③ **2층 GD-8 그림일기 서버이전** ④ KT 질문 D4~D6(보안 요건) — **S7은 KT 요건 확인이 선행**
