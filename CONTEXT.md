...
- ✅ 토대(디자인 토큰) · Landing · Login · ProfileSelect · KidHome · ChatWidget · VideoModal · VideoPlayer · PlaylistModal
- ⏳ Favorites · Games · BadgeCollection · ParentDashboard · Account · AdminPage (라이트 잔존 → 다크 전환 예정)
- ✅ 아바타 재가공(상반신·머리위 여백 통일) — `fix_avatars.py`

## 남은 작업

1. UI 다크 전환 마저 — Favorites · Games · BadgeCollection · ParentDashboard · Account · AdminPage
2. 게임 보너스 시간 수정 (Freddie 메모 `피드백.md`)
3. 결제 연동 (토스페이먼츠 구독) — ⏸ **개인사업자 등록 후 진행** (사업자등록번호 필요 → 현재 보류, 결제 UI는 데모용)
4. 도네이션 결제 — ⏸ (동일 사유로 보류)
5. 관리자 페이지 추가 아이디어 — 룰/필터/정책 분리 관리(키워드 blocklist / 채널 allowlist / 점수 임계값) — 선택
