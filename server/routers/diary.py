...
async def list_entries_by_size(user: dict = Depends(get_current_user)):
    ...
    🔴 정정 (2026-08-12) — 이 독스트링은 원래 *"용량을 차지하는 것은 공유분만이 아니라
       **비공개 일기까지 전부**"* 라고 적혀 있었다. **사실이 아니다.**
       `sql/007_diary_deletion_guarantee.sql:159` 이 명시한다:
         · `share_with_parent` 를 false 로 만드는 코드가 **0건**이고 설정 UI 도 없다
         · **비공개 그림일기는 성립하지 않는다** — 아이 기기는 부모도 만지는 기기다(오너 확정 2026-08-08)
         · 비밀은 **체크인**에만 있다(`DailyCheckin.jsx:545`). 거기선 **저장 자체를 안 해서** 진짜 비밀이다
       그 SQL 은 *"이 컬럼을 보고 '비공개 일기는 부모에게 안 보이는구나' 라고 읽지 말 것"* 이라고
       경고까지 해두었는데, 이 주석이 바로 그 오독을 새로 만들어 냈다.
       ⚠️ **없는 보호를 있다고 적는 것이 아무것도 안 적는 것보다 나쁘다** — 다음 사람이 그걸 믿고 판단한다.
       (실제로 2026-08-12 에 이 주석을 근거로 오너에게 틀린 답을 했다)

    🚨 그래도 내용은 싣지 않는다 — 이유가 '비공개 보호'가 아닐 뿐이다
       이건 **부모 화면**이 부르는 API 이고, 하는 일은 *"무엇을 지울지 고르게 하는 것"* 하나다.
       그 일에 문장·감정·그림 URL 은 필요 없다. 필요 없는 것을 보내지 않는 것이 기본이고,
       나중에 아이가 "이건 나만 볼래"를 고를 수 있게 되면 **이 자리를 고치지 않아도 안전하다.**
       ⇒ 날짜 · 바이트 · 공유 여부, 그 셋뿐이다.
