...
async def update_role(user_id: str, data: RoleRequest, admin: dict = Depends(require_admin)):
    ...
    await write_audit(admin, "역할 변경", target=user_id, detail=f"role → {data.role}")
...
async def update_premium(user_id: str, data: PremiumRequest, admin: dict = Depends(require_admin)):
    if data.grant:
        ...
        await write_audit(admin, "프리미엄 부여", target=user_id)
        ...
        await write_audit(admin, "프리미엄 해제", target=user_id)
