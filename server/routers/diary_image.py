...
#   ⚠️ 아이 그림(전경) 보존이 최우선·불가침(관문=스펙). Sonnet은 색감·분위기(기분/날씨)와 함께,
#      일기 속 '장소'를 배경/여백에만 은은히 반영(바다·놀이터·집 등) — childPick도 배경/여백에 작게.
#      전경(아이가 그린 선·주요 형태) 위에는 절대 덮지 않음(오너 승인 A+B·2026-07-09 스펙 개정).
...
def _continue_system_prompt(gender: str) -> str:
    """보존 우선 편집 프롬프트 조립 — 아이 그림(전경) 절대 보존, 일기는 색감·분위기 + '장소'를 배경/여백에만 반영(childPick도 배경/여백 작게, 전경 불가침)."""
    return (
