...
LIMITS = {
    ...
    #
    # ⚠️ cost_krw 5 는 **추정이다** (Haiku · max_tokens 150~700 · 환율 1,400원 기준).
    #    실측하면 3~8원 사이로 갈릴 수 있고 그러면 총액이 3배까지 달라진다.
    #    → Anthropic 콘솔 실제 청구액으로 재고 나서 이 숫자를 고칠 것(미해결 질문).
    #
    ...
    # ⚠️ cost_krw 는 전부 **추정**이다. 실측 후 고칠 것(미해결 질문).

    # 체크인 반응 (Haiku 160토큰). 세션 1회에 5문항 → 5회 + 재시도 여유
    "checkin_react":  {"per_day": 8, "per_min": 10, "cost_krw": 2,
...
ACCOUNT_LIMITS = {
    ...
    "checkin_react":  {"per_day": 30},   # 아이 4명 × 세션 6회 = 24 + 여유
