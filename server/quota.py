...
LIMITS = {
    ...
    #      월 기본료 90,000원(🔴 사용과 무관한 **월정액**·일할 계산) + 기본 제공 100만 자 + 초과 1,000자당 100원
    #      (2026-08-21 청구서 실측 확정 — "100만 자 무료"가 아니라 **기본료에 포함된 제공량**이다.
    #       7월: 기본료 9만원 → 크레딧 상쇄 / 8월: 크레딧 소진, 키를 지워도 하루 ≈3,000원씩 실비)
    #    ⚠️ 2026-08-21 NCP Application 삭제 — 서비스 해지 상태. 이 항목은 재개 대비로 남긴다(그때 월정액 재개).
    #    문구 평균 20자(실측) → 100만 자 = 5만 회/월 = 하루 1,667회(전체 합). 아이 1명 월 25,261자.
    #    ⇒ cost_krw 2 는 **기본 제공량을 넘겼을 때**의 값이다. 100만 자 안에서는 추가 0원.
    #
