...
### 가족 스케줄러 + AI 에이전트 (SchedulePlanner.jsx / schedules.py)
- **월간 달력** — 아이별 일정/이벤트/음식/상태 기록(CRUD), 기간 일정 가로 바, 날짜 클릭 시 달력 위 팝오버 입력
- **🤖 대화형 AI 에이전트** — 자연어 한 문장으로 일정 처리. "12~18일 모두 지우고 12일에 부산 여행 넣어줘"처럼 **복합명령**을 operations 리스트로 분해해 순서 실행
  - **등록/조회(브리핑)/수정/삭제** 모두 지원
  - 설계 원칙(*사실은 코드가, LLM은 분류만*): LLM은 의도분류+날짜추출만, 실제 CRUD·안내문구는 코드가. 조회는 일정을 DB에서 코드가 읽어 환각 0. 수정/삭제는 대상 1개일 때만 실행(오삭제 방지)
  - 날짜 기준: `today`(상대표현) + `viewMonth`(보고 있는 달 — '12일'처럼 일만 말할 때 기준) 전달
  - 모델: Haiku(비용 절약, 한 줄로 Sonnet 교체 가능)
- 키디 스케줄 인사 — 오늘/내일 일정 분위기 한마디(LLM은 톤만, 사실은 코드 렌더)
