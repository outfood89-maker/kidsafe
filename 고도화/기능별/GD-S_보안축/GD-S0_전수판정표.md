...
### 4-0-4. 관리자 sudo 게이트 — 비밀번호 재확인 (2026-08-05)

> **오너 요구:** *"관리자 권한이 있는 아이디로만 들어갈 수 있어야 하고, 들어가면 다시 재로그인을 해야 하고?"*
> 앞은 `AdminRoute`(4-0-3)로 이미 처리됨. 뒤가 이 항목이다.

**오너 결정:** 비밀번호 재입력 / **30분** 유효 / 실패 시 프로필 선택으로.

#### 왜 "완전 로그아웃 후 재로그인"이 아닌가

로그아웃하면 **앱 전체 세션이 끊긴다.** 관리자 화면 한 번 보려고 나올 때 아이 홈·프로필까지 전부 다시 로그인해야 한다.
업계 표준은 **sudo 모드** — GitHub 설정 변경, AWS 결제 열람에서 *"비밀번호를 다시 입력하세요"* 가 뜨는 그것. **로그인은 유지한 채 그 화면에 들어갈 때만 한 번 더 확인**한다.

#### 핵심 설계 — 메인 세션을 건드리지 않는다

`supabase.auth.signInWithPassword` 를 **메인 클라이언트로** 다시 부르면 새 세션이 발급되고 `onAuthStateChange` 가 발화해 `AuthContext` 가 통째로 다시 돈다.
→ [`utils/supabase.js`](../../../client/src/utils/supabase.js) 에 **검증 전용 클라이언트**를 둔다: `persistSession:false` · `autoRefreshToken:false` · `storageKey` 분리. **검증만 하고 흔적을 남기지 않는다.**

#### 자물쇠 네 겹 (최종)

```
/admin 진입
 → 🔒 ProtectedRoute   로그인 안 했으면 → /login
 → 🔒 AdminRoute       role ≠ admin    → /profiles
 → 🔒 AdminGate        비밀번호 재확인 (30분, sessionStorage)
 → 🔒 서버 require_admin  API 13개 전부 403   ← 진짜 방어선
```

⚠️ **AdminGate 가 막는 것 / 못 막는 것:** 막는다 = 로그인된 기기를 남이 만졌을 때(오너의 실제 위험). 못 막는다 = 작정한 공격자 — 프론트 게이트라 개발자도구로 우회 가능하고 `sessionStorage` 값도 조작 가능하다. **진짜 방어선은 서버 `require_admin` 이며 이 게이트와 무관하게 항상 작동한다.** 이건 '자물쇠 하나 더'다.

- 시도 횟수 **3회** — 오너 결정("실패 시 돌려보내기")을 지키되 오타 한 번에 튕기지 않게 한 절충. 소진 시 `/profiles`.
- `sessionStorage` 사용(localStorage 아님) → **탭을 닫으면 통행증이 사라진다.**

#### 🔴 이 작업에서 팀장이 만든 실수 2건 — 둘 다 테스트가 잡았다

| # | 실수 | 증상 | 원인 | 수정 |
|---|---|---|---|---|
| 1 | 검증용 supabase 클라이언트를 **모듈 최상단에서 즉시 생성** | 테스트 5초 → 147초, 46개 타임아웃 | `utils/supabase.js` 는 거의 모든 화면이 import 한다 → 앱 전역에서 클라이언트가 하나 더 초기화됨 | **지연 생성**(`getVerifyClient()`) — 비밀번호를 입력하는 순간에만 만든다 |
| 2 | 아이콘 2개를 쓰려고 **`react-icons/fa` 배럴 import** | `import` 9.45s → **120.04s**, 37개 타임아웃 | 아이콘 **1,611개**짜리 묶음이 통째로 딸려옴. `AdminGate` 는 `App.jsx` 가 직접 import 하므로 **App 을 쓰는 모든 테스트**가 매번 로드 | **인라인 SVG로 직접 그림.** 의존성 0 |

> **힌트는 코드에 이미 있었다** — 같은 자리(App 이 직접 import 하는 컴포넌트)의 형제인 `ProtectedRoute`·`AdminRoute` 는 **둘 다 react-icons 를 쓰지 않는다.** 왜 안 쓰는지 살폈어야 했다.
> **재발방지:** `App.jsx` 가 직접 import 하는 컴포넌트에서는 배럴 import(`react-icons/*` 등)를 쓰지 말 것. 아이콘 몇 개면 인라인 SVG.

#### 진단 과정에서 팀장이 두 번 헛짚음 (기록)

| 팀장 진단 | 실제 | 바로잡은 계기 |
|---|---|---|
| "Steam 이 CPU 146% 주범" | `ps aux` 의 `%CPU` 는 **프로세스 생애 평균**이지 현재값이 아니다 | **오너 지적** — *"지금 CPU 문제 없는 것 같은데?"* (활성 모니터: 대기 52%) |
| "게임 부하 때문" | **게임이 켜진 채로** 기준선 199개 전부 통과(7.32s) | `git stash` 로 변경을 치우고 **같은 조건에서 기준선 측정** |

> **교훈:** 환경 탓으로 넘기기 전에 **내 변경을 치우고 같은 조건에서 재보는 것**이 유일하게 확실한 판별법이다. 오너가 짚어주지 않았으면 부하 탓으로 덮고 커밋했을 것이고, **앱 로딩이 느려진 채 배포**됐을 것이다.

#### 검증

| 항목 | 결과 |
|---|---|
| [회귀 테스트 신설](../../../client/src/__tests__/admin-gate-sudo.dom.test.jsx) — 9개 | ✅ 확인 전 미노출 / 성공 시 진입 / **🔴 메인 세션 무영향** / 30분 유효·만료 / 실패 시 잔여횟수 / 3회 소진 시 이탈 / 실패 시 통행증 미저장 / 돌아가기 |
| 프론트 테스트 전체 (기본 워커, 게임 켜진 상태) | ✅ **27파일 208테스트, 8.50초** (기준선 199개·7.32초와 동급) |
| 프로덕션 빌드 | ✅ 3.21s |
| 삭제(−) 줄 | ✅ **1줄** — `/admin` 라우트, 같은 줄의 수정판. 로직 삭제 0 |
...
### 4-0-6. 🔴 가짜(mock) 주입 사고 3건 → 공용 도구 신설 (2026-08-05)

> **오너 지시:** *"이거 계속 반복되고 있는 거라면 더 안 그럴 수 있게 최선의 방법을 생각해봐."*

이 세션에서 **하루에 세 번** 같은 종류의 실수를 했다. 처음엔 "두 번"으로 보고했으나 **확인 결과 세 번**이었고, **세 번째가 가장 위험했다.**

| # | 무엇을 했나 | 방향 | 결과 |
|---|---|---|---|
| 1 | `write_audit` 을 **통째로** 교체 — 내부에서 예외를 삼키도록 설계된 함수였다 | 거짓 실패 | 없는 버그(500) → "실패 1건"으로 보고 |
| 2 | `load_dotenv` 를 안 막음 — 로컬 `.env` 가 내 환경변수를 덮어씀 | 거짓 실패 | 배포 상황 재현 실패 → 7건 오탐 |
| 3 | `fake_yt_search` 를 **정의만 하고 어디에도 할당 안 함** | 🔴 **거짓 통과** | 진짜 YouTube 호출 → **검증 안 된 채 통과** + 쿼터 300유닛 |

> 🔴 **3번이 오늘 아침에 배운 "안 던진 질문"과 같은 구조다 — 조용하다.** 통과로 나오니 아무도 다시 안 본다.
> 그리고 결정적으로, 그 테스트는 `CALLS` 기록을 **검사하긴 했다** — `anthropic`·`db_insert` 는 확인하고 **`youtube` 만 빠뜨렸다.** 한 줄이면 잡혔다.

**공통 뿌리:** *가짜가 실제 실행 경로에 제대로 꽂혔는지를 확인하지 않았다.*
1번은 가짜와 **실제 고장 지점** 사이에 방어막 층, 2번은 가짜와 **판정 지점** 사이에 덮어쓰는 층, 3번은 **아무 연결도 없었다.**

#### 대책 — 규칙 + 도구 (오너 선택: C안)

규칙만으로는 또 잊는다(**오늘 하루에 세 번 잊었다**). **도구가 있어야 지켜진다.**
단, 이미 통과한 스크립트 4개는 **건드리지 않았다** — 잘 도는 걸 다시 만지면 거기서 새 실수가 난다(오늘 세 번 당한 패턴).

**신설: [`server/_verify_fakes.py`](../../../server/_verify_fakes.py)** — 검증 스크립트 전용 공용 가짜(서버 런타임 무관, 파일명 `_` 가 그 표시)

| 원칙 | 내용 | 도구 |
|---|---|---|
| ① | **경계에 끼운다** — DB(`sb_*`) · LLM(`anthropic`) · HTTP(`httpx`) · 환경(`load_dotenv`). 그보다 바깥은 중간 층을 건너뛴다 | `FakeDB` · `FakeAnthropic` · `FakeHTTP` · `block_dotenv()` · `reload_app()` |
| ② | **가짜는 호출 기록을 남기고 테스트가 그 기록을 검사한다.** **0번이어야 하는 것도 반드시** | `Recorder.assert_called()` / **`assert_not_called()`** |
| ③ | **결과가 이상하면 코드 말고 배선부터 의심한다** (실패→가짜가 꽂혔나 / 통과→진짜 검증됐나) | — |

⚠️ **`patch()` 는 없는 이름에 주입하면 즉시 `RuntimeError`** 를 낸다. 파이썬이 오타(`search_youtub`)를 조용히 새 속성으로 만들어주는 것이 사고 3을 가능하게 했다 → `setattr` 직접 사용 금지.

#### 검증 — 도구를 만든 것으로 끝내지 않았다

**"그 도구가 그 사고를 실제로 잡느냐"** 를 재현해서 증명했다. → [`verify_fakes_catches.py`](verify_fakes_catches.py) **통과 6 / 실패 0**

| 재현 | 결과 |
|---|---|
| 사고 3 — 오타 난 이름 주입 | ✅ 즉시 `RuntimeError`: *"routers.search.search_youtub 이 존재하지 않습니다"* |
| 사고 3 — 안 꽂힌 채 검사 | ✅ `assert_called` 가 *"http.get 이 한 번도 안 불렸다 — 가짜가 안 꽂혔을 수 있다"* 로 잡음 |
| 사고 1 — 경계(DB)에 끼우기 | ✅ `write_audit` 의 방어막이 **살아있다**(예외가 밖으로 안 샘) + 호출은 기록됨 |
| 사고 2 — `block_dotenv` 미적용 | ✅ `.env` 가 이겨 `IS_DEV=True` (production 을 줬는데 개발 모드) — 재현 실패가 재현됨 |
| 사고 2 — `block_dotenv` 적용 | ✅ `IS_DEV=False` — 배포 정확히 재현 |

`_verify_fakes.py` 자체 점검도 내장(`python _verify_fakes.py` → 9/9 통과).

**하네스 등재:** `CLAUDE.md` 「가짜(mock) 주입 3원칙」
