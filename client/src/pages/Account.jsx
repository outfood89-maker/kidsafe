...
export default function Account() {
  ...
  return (
    <div className="min-h-screen w-full" style={{ backgroundColor: "#F8F7F2" }}>
      {/* 상단 네비 */}
      <header className="sticky top-0 z-50 bg-white" style={{ borderBottom: "0.5px solid #E4EAE0" }}>
        <div className="mx-auto flex h-14 max-w-2xl items-center justify-between px-4">
          ...
          <h1 className="text-base font-medium" style={{ color: "#2C3528" }}>내 계정</h1>
      ...
      <div className="mx-auto max-w-2xl px-4 py-6">
        ...
        <div className="flex mb-6 rounded-xl p-1" style={{ backgroundColor: "#F0F5ED" }}>
          {[["info", "👤 내 계정"], ["membership", "💎 멤버십"]].map(([key, label]) => (
            <button
              ...
              style={{ backgroundColor: tab === key ? "#6DAB60" : "transparent", color: tab === key ? "white" : "#6B7A65" }}
            >
              {label}
            </button>
          ))}
        ...
        {tab === "info" && (
          <div className="flex flex-col gap-4">
            {/* 기본 정보 */}
            <div className="bg-white p-5 rounded-2xl" style={{ border: "0.5px solid #E4EAE0" }}>
              <h2 className="text-base font-semibold mb-4" style={{ color: "#2C3528" }}>기본 정보</h2>

              {/* 이메일 (읽기 전용) */}
              <div className="mb-4">
                <label className="block text-xs font-medium mb-1.5" style={{ color: "#6B7A65" }}>이메일</label>
                <div className="flex items-center gap-2 rounded-xl px-3 py-3" style={{ backgroundColor: "#F8F7F2", border: "1px solid #E4EAE0" }}>
                  <FaEnvelope style={{ color: "#9BA89A" }} />
                  <span className="text-sm" style={{ color: "#9BA89A" }}>{user?.email}</span>
                </div>
              </div>

              {/* 이름 수정 */}
              <div>
                <label className="block text-xs font-medium mb-1.5" style={{ color: "#6B7A65" }}>보호자 이름</label>
                <div className="flex gap-2">
                  <div className="flex flex-1 items-center gap-2 rounded-xl px-3" style={{ backgroundColor: "#F8F7F2", border: "1px solid #E4EAE0" }}>
                  ...
                  <button
                    ...
                    className="rounded-xl px-4 text-sm font-semibold text-white transition hover:opacity-90 disabled:opacity-60"
                    style={{ backgroundColor: "#6DAB60", minWidth: "56px" }}
                  >
            ...
            {/* 비밀번호 변경 */}
            <div className="bg-white p-5 rounded-2xl" style={{ border: "0.5px solid #E4EAE0" }}>
              <h2 className="text-base font-semibold mb-4" style={{ color: "#2C3528" }}>비밀번호 변경</h2>
              <form onSubmit={handleChangePassword} className="flex flex-col gap-3">
                ...
                {pwError && <p className="text-xs px-1" style={{ color: "#C84B47" }}>⚠️ {pwError}</p>}
                <button
                  type="submit"
            ...
            {/* 로그아웃 */}
            <div className="bg-white p-5 rounded-2xl" style={{ border: "0.5px solid #E4EAE0" }}>
              <h2 className="text-base font-semibold mb-3" style={{ color: "#2C3528" }}>로그아웃</h2>
              <p className="text-sm mb-4" style={{ color: "#9BA89A" }}>현재 기기에서 로그아웃합니다.</p>
              <button
            ...
            {/* 회원 탈퇴 */}
            <div className="bg-white p-5 rounded-2xl" style={{ border: "1px solid #FDD" }}>
              <h2 className="text-base font-semibold mb-1" style={{ color: "#C84B47" }}>회원 탈퇴</h2>
              <p className="text-xs mb-4" style={{ color: "#9BA89A" }}>탈퇴하면 모든 자녀 프로필·시청기록이 삭제되며 복구할 수 없어요.</p>
              <div className="flex gap-2">
                <input
                  type="text"
                  value={deleteConfirm}
                  ...
                  style={{ backgroundColor: "#F8F7F2", border: "1px solid #E4EAE0", color: "#2C3528" }}
                />
                <button
                  ...
                  style={{ backgroundColor: "#C84B47", color: "white" }}
                >
                  탈퇴
                </button>
              </div>
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="bg-white p-5 rounded-2xl" style={{ border: "0.5px solid #E4EAE0" }}>
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-base font-semibold" style={{ color: "#2C3528" }}>현재 플랜</h2>
                <span className="rounded-full px-3 py-1 text-xs font-semibold" style={{ backgroundColor: "#F0F5ED", color: "#6DAB60" }}>
                  Free
                </span>
              </div>
              <ul className="flex flex-col gap-2 mb-4">
                {[
                  ...
                  <li key={text} className="flex items-center gap-2 text-sm" style={{ color: "#6B7A65" }}>
                    <span>{icon}</span>{text}
                  </li>
                ))}
              </ul>
            ...
            >
              <span className="absolute top-4 right-4 rounded-full px-2.5 py-1 text-xs font-bold" style={{ backgroundColor: "#EF9F27", color: "#2C3528" }}>
                가장 인기
              </span>
              ...
              <ul className="flex flex-col gap-2 mb-5">
                {[
                  ...
                  <li key={text} className="flex items-center gap-2 text-sm text-white">
                    <span className="flex h-4 w-4 items-center justify-center rounded-full text-xs" style={{ backgroundColor: "#6DAB60" }}>✓</span>
                    {text}
                  </li>
                ))}
              </ul>
