...
// ── 운영 주체 표기 (2026-08-08 오너 확정 — '실명 없이') ──
// 법 제30조가 요구하는 것은 "보호책임자의 **성명 또는** 담당부서의 명칭과 연락처" 다.
// 부서명 + 연락처로 요건이 충족되므로 오너 실명·주소·전화를 공개 사이트에 올리지 않는다.
// ⚠️ 사업자 등록과는 무관하다 — 사업자등록번호는 처리방침 법정 기재사항이 아니다(GD-40 정정).
// 🔴 정식 출시로 상호가 생기면 OPERATOR_NAME 을 상호로 교체할 것.
// ⚠️ 없는 부서 이름을 지어내지 말 것 (2026-08-08 오너 지적으로 정정).
//    처음엔 "개인정보 보호 담당" 이라고 적었는데, 1인 운영이라 그런 부서가 실재하지 않는다.
//    오너가 "그래서 담당이 누군데?" 라고 물었다 — 오너가 헷갈리면 부모는 더 헷갈린다.
//    ⇒ 실제 구조를 그대로 적는다. 실명만 비공개일 뿐 역할과 연락처는 명확하다.
const OPERATOR_NAME = "키디 (1인 개발·운영)";    // 운영 주체
const DPO_NAME = "키디 운영자";                 // 보호책임자 — 1인이므로 운영자 본인이 겸한다
const EFFECTIVE_DATE = "2026-08-08";           // 시행일 — 채우면 상단 '준비 중' 배너가 자동으로 사라진다
...
// 🔴 이 값을 바꿀 때 **문구가 갈리는 자리 4곳**을 전부 확인할 것 (privacy-policy 테스트가 강제한다):
//    ① 먼저 알려드립니다 3번 항목  ② 한눈에 보기 📓 카드  ③ 제1조 '기기에만 남는 것'  ④ 제5조 본문
//    2026-08-08 에 ①이 분기 없이 하드코딩돼 있어, 켜면 맨 위와 제5조가 서로 모순될 뻔했다.
const DIARY_SERVER_LIVE = true;
          ...
          <div className="space-y-2.5">
            {[
              ...
              },
              {
                ...
                // 🔴 2026-08-08 — 여기엔 DIARY_SERVER_LIVE 분기가 없었다.
                //    켰을 때 맨 위는 "이 기기에서만"이라 하고 제5조는 "서버에 저장됩니다"라고 해
                //    같은 문서가 서로 다른 말을 할 뻔했다. 기존 검사는 요약카드·제5조만 봐서 못 잡았다.
                to: "#s5",
                t: DIARY_SERVER_LIVE
                  ? "‘가족 책장’을 켜지 않으면 그림일기는 이 기기에서만 보입니다"
                  : "그림일기는 지금 이 기기에서만 보입니다",
                d: DIARY_SERVER_LIVE
                  ? "기본값은 ‘이 기기에만 저장’입니다. 보호자가 ‘어디서나 열리는 가족 책장’을 켜신 아이만 서버에도 저장되어 다른 기기에서 열립니다. 켜지 않으신 동안에는 아이폰이든 안드로이드든 PC든 아래가 그대로 적용됩니다."
                  : "아이폰이든 안드로이드든 PC든 마찬가지입니다. 브라우저는 저장한 내용을 다른 기기와 공유하지 않습니다.",
                li: [
