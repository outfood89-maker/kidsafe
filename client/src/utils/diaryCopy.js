...
export const DIARYFLOW_TOUR = {
  ...
  stations: [
    ...
    "아이가 고른 답들이 이렇게 한 편의 글로 엮여요. 직접 쓰지 않아도, 자기 이야기가 글이 되죠.", // ② 완성된 글
    "그림도 함께 담겨요. 키디가 그려주거나, 아이가 직접 그린 그림을 이어서 완성할 수도 있어요.", // ③ 그림 자리
    "완성한 일기는 '간직하기'로 책장에 보관돼요. 그리고 그 마음이 부모님 리포트로 조용히 전해지죠.", // ④ 간직 버튼
  ],
};

// 방식 S 데모 시드 — DiaryFlow를 result(완성)에 읽기전용으로 열 때 채우는 예시 데이터(팀장 스탬프). 서버호출 0(로컬 자산).
export const DIARYFLOW_TOUR_SEED = {
  sentences: ["오늘은 날씨가 맑았어.", "친구랑 놀이터에서 그네를 탔어.", "정말 신나는 하루였어!"],
  weatherKey: "sunny",
  moodEmoji: "😄",
  cardImageUrl: "/images/logo/icon_light_256.png", // 그림 자리 스탠드인 = 앱 아이콘(로컬·외부GET 0). 오너 지시 교체(symbol→app icon).
};
