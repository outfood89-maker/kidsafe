...
import ParentDashboard, { tabTourRange } from "../pages/ParentDashboard";
...
// ── 항목1: 탭별 "?" 스코프 서브투어 ──
describe("항목1 T1 — tabTourRange (탭별 연속 구간)", () => {
  it("각 탭이 §1 표의 인덱스 범위와 정확히 일치, 정거장 없는 탭은 null", () => {
    expect(tabTourRange("kiddy")).toEqual({ start: 0, end: 1 });
    expect(tabTourRange("shelf")).toEqual({ start: 2, end: 2 });
    expect(tabTourRange("schedule")).toEqual({ start: 3, end: 4 });
    expect(tabTourRange("safety")).toEqual({ start: 5, end: 7 });
    expect(tabTourRange("analysis")).toEqual({ start: 8, end: 9 });
    expect(tabTourRange("history")).toEqual({ start: 10, end: 10 });
    expect(tabTourRange("children")).toEqual({ start: 11, end: 12 });
    expect(tabTourRange("overview")).toBeNull(); // 정거장 없는 탭 → "?" 숨김
  });
});

describe("항목1 T2 — 탭 스코프 서브투어(safety: 정거장 5~7만)", () => {
  it("안전 탭 '?' → 상대 1/3에서 시작, next 끝에서 종료 CTA→종료, 첫 정거장엔 '이전' 없음(하한)", async () => {
    render(<ParentDashboard />);
    await waitFor(() => expect(screen.queryByText("불러오는 중...")).toBeNull());
    // 안전 알림 탭으로 전환(비투어) → 그 탭 "?" 클릭
    fireEvent.click(screen.getByRole("button", { name: /안전 알림/ }));
    fireEvent.click(await screen.findByTestId("tab-tour-btn"));

    const overlay = screen.getByTestId("tour-overlay");
    // ⑤a(전역 index 5)=safety 첫 정거장, 탭 내 상대 1/3
    expect(within(overlay).getByText(PARENT_TOUR.stations[5])).toBeTruthy();
    expect(within(overlay).getByText("1 / 3")).toBeTruthy();
    expect(within(overlay).queryByText(PARENT_TOUR.nav.prev)).toBeNull(); // 상대 step0 → '이전' 미표시(하한: 전 탭으로 안 넘어감)

    // next×2 → ⑤c(index 7) 상대 3/3 + 종료 CTA
    fireEvent.click(within(overlay).getByText(PARENT_TOUR.nav.next));
    expect(within(screen.getByTestId("tour-overlay")).getByText("2 / 3")).toBeTruthy();
    fireEvent.click(within(screen.getByTestId("tour-overlay")).getByText(PARENT_TOUR.nav.next));
    const last = screen.getByTestId("tour-overlay");
    expect(within(last).getByText(PARENT_TOUR.stations[7])).toBeTruthy();
    expect(within(last).getByText("3 / 3")).toBeTruthy();
    expect(within(last).getByText(PARENT_TOUR.exitCta)).toBeTruthy();

    // 범위 끝 '다음'(=종료 CTA) → 투어 종료(코치마크 사라짐)
    fireEvent.click(within(last).getByText(PARENT_TOUR.exitCta));
    await waitFor(() => expect(screen.queryByTestId("tour-overlay")).toBeNull());

    // 항목1 후속: 탭 서브투어 종료 후 '그 탭(safety)'에 머묾 — 자녀설정으로 튀지 않음
    await waitFor(() => expect(screen.getByRole("heading", { name: /안전 알림/ })).toBeTruthy());
    expect(screen.queryByRole("heading", { name: /자녀 설정/ })).toBeNull();
  });
});

describe("항목1 T3 — 전체투어 종료는 자녀설정 착지(무회귀)", () => {
  it("헤더 '둘러보기'(전체 range) → 마지막 종료 CTA → 자녀 설정 탭 착지(기존 동작 유지)", async () => {
    render(<ParentDashboard />);
    await waitFor(() => expect(screen.queryByText("불러오는 중...")).toBeNull());
    fireEvent.click(screen.getByTestId("tour-header-btn")); // 전체 둘러보기(range 0~12)
    await screen.findByTestId("tour-overlay");
    // 13정거장 — next 12회로 마지막(index 12)까지
    for (let i = 0; i < PARENT_TOUR.stations.length - 1; i++) {
      fireEvent.click(within(screen.getByTestId("tour-overlay")).getByText(PARENT_TOUR.nav.next));
    }
    const last = screen.getByTestId("tour-overlay");
    expect(within(last).getByText(PARENT_TOUR.exitCta)).toBeTruthy(); // 전체 총 13/13 종료 CTA
    fireEvent.click(within(last).getByText(PARENT_TOUR.exitCta));
    await waitFor(() => expect(screen.queryByTestId("tour-overlay")).toBeNull());
    // 전체투어=exitTour(true) → 자녀 설정 탭 착지(기존 온보딩 동작 그대로)
    await waitFor(() => expect(screen.getByRole("heading", { name: /자녀 설정/ })).toBeTruthy());
  });
});
