...
const SYSTEM_PROMPT =
  ...
  "장면 변환 규칙: 일기 문장에 있는 소재만 사용(문장 밖 창작 금지)·날씨는 하늘로·한 일은 중심 행동·인물은 일반화된 아이/어른·기분은 표정과 팔레트 온도·그림 참여 답은 반드시 등장·키디(작은 초록 아기공룡) 동반.\n\n" +
...
const FALLBACK_PROMPT = `${STYLE_BLOCK} A happy little child playing outside on a sunny day with their mother, a playground slide in the scene, a small green baby dinosaur friend nearby, bright sun in the sky. ${SAFETY_BLOCK}`;
...
const delays = [];
let okCount = 0;
console.log(`PoC r2 — ${IMAGE_MODEL} × ${N}건 (Sonnet 변환 ${ANTHROPIC_KEY ? "ON" : "OFF(폴백 프롬프트)"})\n`);
for (let i = 1; i <= N; i++) {
...
console.log(`PNG: ${OUT}/  → 오너 육안 검수: ①왜곡(문장 밖 소재) ②미끄럼틀 10/10 등장 ③텍스트0 ④(흐린날 톤=별도 sad 예시 필요) ⑤그림체 일관성`);
